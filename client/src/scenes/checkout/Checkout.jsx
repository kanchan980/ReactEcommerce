...
// ✅ Initialize Stripe once with env key
...
const Checkout = ({ cartItems = [], totalPrice = 0, onBack }) => {
  ...
  const mapProductsForOrder = (items) => {
    ...
    return items.map((item, idx) => {
      ...
      if (!productData?.id) {
        throw new Error(`Invalid product at position ${idx + 1}`);
      }
      ...
      if (price === null || price === undefined) {
        throw new Error(`Price missing for "${attributes.name}"`);
      }
      if (typeof price === "object") {
        price = price.amount ?? price.value ?? (() => {
          throw new Error(`Invalid price for "${attributes.name}"`);
        })();
      }
      ...
      if (isNaN(quantity) || quantity < 1) {
        throw new Error(`Invalid quantity for "${attributes.name}"`);
      }
  ...
  const placeOrder = async (values, actions) => {
    ...
    try {
      ...
      const stripeSessionId =
        data.stripeSessionId || data.data?.attributes?.stripeSessionId;
    ...
    <Box width="90%" maxWidth="800px" m="50px auto" pt="50px">
      ...
      {orderData && (
        <Box mt={5} p={3} border="1px solid #ccc" borderRadius={2}>
          ...
          <Typography mb={1}>
            <strong>Order ID:</strong> {orderData.data?.id || "N/A"}
          </Typography>
          <Typography mb={3}>
